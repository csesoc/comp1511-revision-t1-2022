...
- Coding the answer up syntaxically right, with pointers
...
`x`, `y` and `z` are integer pointers and so to access the actual integer value they point to, we need to dereference it. This syntaxically means putting a `*` in front of the variable, so we can compare the values of `x` and `z` like this:
