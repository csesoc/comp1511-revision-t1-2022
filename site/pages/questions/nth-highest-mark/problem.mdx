...
```
Input: 5 7 45 63 67 97 53 36 74
Output: 53
Explanation: The first input is Bobby's rank. The second input is the number of marks given. The remaining inputs are the unordered scores. Since Bobby placed 5th, you must find the 5th highest score, which is 53.
```

**Example 2**

```
Input: 4 4 90 88 88 70
Output: 70
Explanation: We are looking for the 4th highest number (first input). We know there are 4 marks given (second input). While 70 is not the 4th highest unique number, since there are two 88 marks 70 gets pushed down to the 4th highest mark.
```
